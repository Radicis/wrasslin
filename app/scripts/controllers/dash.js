'use strict';
angular.module('Wrasslin').controller('DashCtrl', function($scope, $window, $ionicModal, $ionicLoading, $state, Points, Events, Matches, Votes, Auth, $firebaseObject, $firebaseArray, $firebaseAuth, $ionicPopup) {
  ...
  Events.getRecentActive().then(function(){
    $scope.notify = 0;
  ...
  matchRef.on('child_added', function(){
    $scope.notify = $scope.notify+1;
    $scope.populateEventInfo();
  });

  var votesRef = ref.child('votes');
  votesRef.on('child_added', function(){
    $scope.populateEventInfo();
  });
